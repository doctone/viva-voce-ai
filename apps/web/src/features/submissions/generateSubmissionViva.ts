...
const structuredVivaQuestionsSchema = z
  ...
  .superRefine((value, ctx) => {
    const categories = [
      ...
      ['argumentation_and_reasoning', value.questions.argumentationAndReasoning],
...
export type StructuredVivaQuestions = z.infer<typeof structuredVivaQuestionsSchema>
...
export type SubmissionVivaGenerationRepository = {
  getSubmission: (submissionId: string) => Promise<SubmissionForGeneration | null>
  replaceQuestions: (
    submissionId: string,
...
async function generateStructuredVivaWithAiSdk(
  ...
  const modelName = process.env.AI_VIVA_MODEL ?? 'gpt-4.1-mini'
...
export async function generateSubmissionVivaAnalysis({
  ...
  try {
    ...
    const normalizedQuestions = normalizeStructuredQuestions(structuredQuestions)
...
export function createSupabaseSubmissionVivaGenerationRepository(): SubmissionVivaGenerationRepository {
  return {
    async getSubmission(submissionId) {
      ...
      if (error) {
        throw new Error('We could not load the submission for viva generation.')
      }
    ...
    async replaceQuestions(submissionId, questions) {
      ...
      const { error: insertError } = await supabase.from('viva_questions').insert(
